...
% Pour le lien vers un doc à compiler AleaTeX
% #1 --> Texte par défaut
% Par défaut #1 est le smiley yeux etoilés + smiley link + texte mais on peut le remplacer par la ref mathalea
% #2 --> url
\newcommand{\hrefAleaTeX}[2][\emoji{star-struck} \emoji{link} \myUrlColorStyle{S'entraîner avec} \aleatexLogo]{
    \qrcode[hyperlink,height=0.4in]{#2}\hspace*{2mm}\href{#2}{#1}
}
...
% Pour les liens sesamaths
% #1 --> emojis par défaut
% Par défaut #1 est le movie-camera + link mais on peut le remplacer par la ref mathalea
% #2 --> atome
% #3 --> ordre
\newcommand{\hrefLienSesamath}[3][\emoji{link}]{
    \qrcode[hyperlink,height=0.4in]{#2}\hspace*{2mm}\href{https://manuel.sesamath.net/numerique/diapo.php?atome=#2\&ordre=#3}{#1 \myUrlColorStyle{https://manuel.sesamath.net/numerique/diapo.php?atome=#2\&ordre=#3}}
}